...
public class SearchFileFullScreen extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
...
//        fileImageGridView.addItemDecoration(new DividerItemDecoration(FullScreen.this, DividerItemDecoration.VERTICAL_LIST));
        fileImageGridView.addItemDecoration(new DividerItemDecoration(SearchFileFullScreen.this, DividerItemDecoration.HORIZONTAL_LIST));
        ...
        etInstructions.setClickable(false);
        etInstructions.setFocusableInTouchMode(false);
        etStatus.setClickable(false);
        etStatus.setFocusableInTouchMode(false);
    ...
    private void submitStatus() {
        ...
        if (spStatus.getSelectedItemPosition() == 2 || spStatus.getSelectedItemPosition() == 3) {
            if (etStatus.getText().toString().isEmpty()) {
                showAlertDialog(SearchFileFullScreen.this, "Please enter comments", false);
            } else if (ConnectionDetector.isConnectedToInernet(SearchFileFullScreen.this)) {
                AsyncCallWS task = new AsyncCallWS();
                task.execute();
            } else {
                showAlertDialog(SearchFileFullScreen.this, "Please check internet service", false);
            }
        } else if (ConnectionDetector.isConnectedToInernet(SearchFileFullScreen.this)) {
            AsyncCallWS task = new AsyncCallWS();
            task.execute();
    ...
    private class AsyncCallWS extends AsyncTask<String, Void, String> {
        ...
        protected void onPreExecute() {
            dialog = new ProgressDialog(SearchFileFullScreen.this);
                ...
                if (!result.equalsIgnoreCase("99")) {
                    if (!result.equalsIgnoreCase("No Record found")) {
                        ...
                        handler.postDelayed(new Runnable() {
                            public void run() {
                                if (SearchFileFullScreen.this != null/* && !isFinishing()*/) {
                                    dialog.dismiss();
                                    if (result.equals("1")) {
                                        showAlertDialogFinish(SearchFileFullScreen.this, "Submit success");
                                    } else if (result.equals("0")) {
                                        showAlertDialog(SearchFileFullScreen.this, "Submit Failed", false);
                                    }
                        ...
                        dialog.dismiss();
                        showAlertDialog(SearchFileFullScreen.this, "No file uploaded yet.", false);
                    }
                } else {
    ...
    public void showDialogRetry(String Message) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            builder = new AlertDialog.Builder(SearchFileFullScreen.this, android.R.style.Theme_DeviceDefault_Light_Dialog_NoActionBar);
        } else {
            builder = new AlertDialog.Builder(SearchFileFullScreen.this);
        }
//        builder.setTitle("Queue Management System");
        ...
        builder.setNegativeButton("Retry", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                if (ConnectionDetector.isConnectedToInernet(SearchFileFullScreen.this)) {
                    submitStatus();
                } else {
                    ShowDialog.showAlertDialog(SearchFileFullScreen.this, "Please check internet service", false);
                }
            }
        });
        builder.show();
