...
public class Tools {
    ...
    public static void displayImageRound(final Context ctx, final ImageView img, String url) {
        try {
            Glide.with(ctx).asBitmap().load(url).placeholder(R.drawable.app_logo).centerCrop().into(new BitmapImageViewTarget(img) {
                @Override
                protected void setResource(Bitmap resource) {
                    RoundedBitmapDrawable circularBitmapDrawable = RoundedBitmapDrawableFactory.create(ctx.getResources(), resource);
                    circularBitmapDrawable.setCircular(true);
    ...
    public static String getFormattedDateSimple1(Long dateTime) {
